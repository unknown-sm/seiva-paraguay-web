...
export interface CountingAdapter extends EcommerceAdapter {
  calls: { searchProducts: number; getProduct: number; getSalesReport: number; getInventoryReport: number };
}
...
export function buildDeps(opts?: { rateLimit?: number; countingAdapter?: boolean }): RunToolDeps & { adapter: CountingAdapter } {
  ...
  const calls = { searchProducts: 0, getProduct: 0, getSalesReport: 0, getInventoryReport: 0 };
